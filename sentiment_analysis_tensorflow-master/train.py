...
tf.flags.DEFINE_integer('n_samples', 16877,#None,
...
tf.flags.DEFINE_integer('batch_size', 512,
                        'Batch size')
tf.flags.DEFINE_integer('train_steps', 6000,
                        'Number of training steps')
tf.flags.DEFINE_integer('hidden_size', 32,#75,
...
file_re = open('record_lyx_fileout43.txt', 'a')
# Prepare summaries
...
for i in range(FLAGS.train_steps):
    ...
    train_loss, _a, acc_train, summary = sess.run([nn.loss, nn.train_step, nn.accuracy, nn.merged],
    ...
    file_re = open('record_lyx_fileout43.txt', 'a')
    file_re.writelines('{0}/{1} train loss: {2:.4f} (accuracy {3:.4f})\n'.format(i + 1, FLAGS.train_steps, train_loss, acc_train))
    file_re.close()
    print('{0}/{1} train loss: {2:.4f} (accuracy {3:.4f})'.format(i + 1, FLAGS.train_steps, train_loss, acc_train))
    ...
    if (i + 1) % FLAGS.validate_every == 0:
        ...
        file_re.writelines('   validation loss: {0:.4f} (accuracy {1:.4f})\n'.format(val_loss, accuracy))
        file_re.close()
        '''
...
file_re.writelines('Model saved in: {0}\n'.format(model_dir))
file_re.close()
