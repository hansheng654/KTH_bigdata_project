...
[y_train,X_raw_train,X_train_sparse],[y_val,X_raw_val,X_val_sparse],[y_test,X_raw_test,X_test_sparse] = get_sparse_data(max_df= 0.995,min_df=0.0001)
...
def check_acc(trained_clf,clf_name,val_acc = False):
    ...
    result_sk = np.array(list(map(int,result_sk)))
    acc = np.mean(result_sk == true_label) *100.
    print("Testing Accuracy, %s with tf-idf: %.2f%% \n" % (clf_name,acc))
...
#    check_acc(text_clf,"SGD",True)
    ...
    text_clf.fit(X_train_sparse,y_train)
    check_acc(text_clf,"Logistic Regression",False)
    
#will never finish if using ngram     
